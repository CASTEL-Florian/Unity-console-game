...
namespace Shmup.Enemies
{
    internal class Tank : IEnemy
    {
        ...
        internal static int XMax = new[]
        {
            spriteDown.Max(s => s.Length), spriteUp.Max(s => s.Length), spriteLeft.Max(s => s.Length),
            spriteRight.Max(s => s.Length),
        }.Max();

        internal static int YMax =
            new[] { spriteDown.Length, spriteUp.Length, spriteLeft.Length, spriteRight.Length, }.Max();
        ...
        public void Update()
        {
            int xDifToPlayer = (int)ShmupProgram.player.X - (int)X;
            int yDifToPlayer = (int)ShmupProgram.player.Y - (int)Y;
